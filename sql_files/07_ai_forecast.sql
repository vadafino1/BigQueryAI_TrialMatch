...
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.enrollment_total_d-- ============================================================================
...
CREATE OR REPLACE MODEL `${PROJECT_ID}.${DATASET_ID}.arima_total_enrollment_model`
OPTIONS(
...
FROM `${PROJECT_ID}.${DATASET_ID}.enrollment_total_daily`;

-- Generate forecasts using ML.FORECAST
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.forecast_total_enrollment` AS
SELECT
  forecast_timestamp,
  forecast_value,
  standard_error,
...
FROM ML.FORECAST(
  MODEL `${PROJECT_ID}.${DATASET_ID}.arima_total_enrollment_model`,
  STRUCT(
...
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.enrollment_by_trial` AS
WITH patient_trial_timeline AS (
  ...
  FROM `${PROJECT_ID}.${DATASET_ID}.discharge_summaries_2025` ds
  LEFT JOIN `${PROJECT_ID}.${DATASET_ID}.hybrid_match_scores` mc
...
CREATE OR REPLACE MODEL `${PROJECT_ID}.${DATASET_ID}.arima_by_trial_model`
OPTIONS(
  model_type = 'ARIMA_PLUS',
  time_series_timestamp_col = 'enrollment_date',
...
SELECT
  ...
  trial_id,
  daily_enrollments
FROM `${PROJECT_ID}.${DATASET_ID}.enrollment_by_trial`;

-- Generate forecasts by trial using ML.FORECAST
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.forecast_by_trial` AS
SELECT
  trial_id,
  forecast_timestamp,
  ...
  prediction_interval_upper_bound
FROM ML.FORECAST(
  MODEL `${PROJECT_ID}.${DATASET_ID}.arima_by_trial_model`,
  STRUCT(
    30 AS horizon,
...
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.patient_engagement_series` AS
WITH patient_activity AS (
  ...
  LEFT JOIN `${PROJECT_ID}.${DATASET_ID}.discharge_summaries_2025` ds
...
CREATE OR REPLACE MODEL `${PROJECT_ID}.${DATASET_ID}.arima_patient_engagement_model`
OPTIONS(
  model_type = 'ARIMA_PLUS',
...
SELECT
  ...
  patient_id,
  engagement_score
FROM `${PROJECT_ID}.${DATASET_ID}.patient_engagement_series`;

-- Forecast patient engagement
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.forecast_patient_engagement` AS
SELECT
  patient_id,
  forecast_timestamp,
...
FROM ML.FORECAST(
  MODEL `${PROJECT_ID}.${DATASET_ID}.arima_patient_engagement_model`,
  STRUCT(
...
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.site_recruitment_series` AS
WITH patient_geographic_patterns AS (
  ...
  FROM `${PROJECT_ID}.${DATASET_ID}.patient_profile` pp
  JOIN `${PROJECT_ID}.${DATASET_ID}.discharge_summaries_2025` ds
    ON pp.patient_id = ds.patient_id
  WHERE ds.admission_date_2025 IS NOT NULL
...
CREATE OR REPLACE MODEL `${PROJECT_ID}.${DATASET_ID}.arima_site_performance_model`
OPTIONS(
  model_type = 'ARIMA_PLUS',
...
SELECT
  ...
  site_id,
  recruitment_rate
FROM `${PROJECT_ID}.${DATASET_ID}.site_recruitment_series`;

-- Forecast site recruitment performance
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.forecast_site_performance` AS
SELECT
  site_id,
  forecast_timestamp,
...
FROM ML.FORECAST(
  MODEL `${PROJECT_ID}.${DATASET_ID}.arima_site_performance_model`,
  STRUCT(
...
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.trial_progress_series` AS
WITH real_trial_enrollment AS (
  ...
  FROM `${PROJECT_ID}.${DATASET_ID}.hybrid_match_scores` mc
  JOIN `${PROJECT_ID}.${DATASET_ID}.discharge_summaries_2025` ds
...
CREATE OR REPLACE MODEL `${PROJECT_ID}.${DATASET_ID}.arima_trial_completion_model`
OPTIONS(
  model_type = 'ARIMA_PLUS',
...
SELECT
  ...
  trial_id,
  cumulative_enrolled
FROM `${PROJECT_ID}.${DATASET_ID}.trial_progress_series`;

-- Forecast trial completion
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.forecast_trial_completion` AS
WITH forecast_data AS (
  SELECT
    ...
    prediction_interval_upper_bound
  FROM ML.FORECAST(
    MODEL `${PROJECT_ID}.${DATASET_ID}.arima_trial_completion_model`,
    STRUCT(
...
CREATE OR REPLACE MODEL `${PROJECT_ID}.${DATASET_ID}.arima_enrollment_model`
OPTIONS(
  model_type = 'ARIMA_PLUS',
  time_series_timestamp_col = 'enrollment_date',
  time_series_data_col = 'total_daily_enrollments',
...
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.ml_forecast_enrollment` AS
SELECT
  forecast_timestamp,
  forecast_value,
  standard_error,
  ...
  prediction_interval_upper_bound
FROM ML.FORECAST(
  MODEL `${PROJECT_ID}.${DATASET_ID}.arima_enrollment_model`,
  STRUCT(
...
CREATE OR REPLACE VIEW `${PROJECT_ID}.${DATASET_ID}.v_forecast_accuracy` AS
WITH forecast_metrics AS (
  ...
  FROM `${PROJECT_ID}.${DATASET_ID}.forecast_total_enrollment`
  ...
  SELECT
    ...
    MIN(prediction_interval_lower_bound) AS min_lower_bound,
    MAX(prediction_interval_upper_bound) AS max_upper_bound
  FROM `${PROJECT_ID}.${DATASET_ID}.ml_forecast_enrollment`
)
SELECT
  forecast_method,
...
CREATE OR REPLACE TABLE `${PROJECT_ID}.${DATASET_ID}.forecast_executive_summary` AS
WITH enrollment_outlook AS (
  ...
  FROM `${PROJECT_ID}.${DATASET_ID}.forecast_by_trial`
...
retention_outlook AS (
  ...
  FROM `${PROJECT_ID}.${DATASET_ID}.forecast_patient_engagement`
),
site_outlook AS (
  SELECT
  ...
  FROM `${PROJECT_ID}.${DATASET_ID}.forecast_site_performance`
)
SELECT
  forecast_type,
  entities_forecasted,
...
WITH validation_results AS (
  SELECT 'forecast_total_enrollment' AS table_name,
         (SELECT COUNT(*) FROM `${PROJECT_ID}.${DATASET_ID}.forecast_total_enrollment`) AS row_count
  UNION ALL
  SELECT 'forecast_by_trial',
         (SELECT COUNT(*) FROM `${PROJECT_ID}.${DATASET_ID}.forecast_by_trial`)
  UNION ALL
  SELECT 'forecast_patient_engagement',
         (SELECT COUNT(*) FROM `${PROJECT_ID}.${DATASET_ID}.forecast_patient_engagement`)
  UNION ALL
  SELECT 'forecast_site_performance',
         (SELECT COUNT(*) FROM `${PROJECT_ID}.${DATASET_ID}.forecast_site_performance`)
  UNION ALL
  SELECT 'forecast_trial_completion',
         (SELECT COUNT(*) FROM `${PROJECT_ID}.${DATASET_ID}.forecast_trial_completion`)
  UNION ALL
  SELECT 'ml_forecast_enrollment',
         (SELECT COUNT(*) FROM `${PROJECT_ID}.${DATASET_ID}.ml_forecast_enrollment`)
)
SELECT
...
SELECT
  ...
  CASE
    WHEN (SELECT COUNT(DISTINCT trial_id) FROM `${PROJECT_ID}.${DATASET_ID}.forecast_by_trial`) > 10
  ...
  CASE
    WHEN (SELECT COUNT(*) FROM `${PROJECT_ID}.${DATASET_ID}.ml_forecast_enrollment`) > 0
  ...
  CONCAT(
    (SELECT COUNT(DISTINCT trial_id) FROM `${PROJECT_ID}.${DATASET_ID}.forecast_by_trial`), ' trials, ',
    (SELECT COUNT(DISTINCT patient_id) FROM `${PROJECT_ID}.${DATASET_ID}.forecast_patient_engagement`), ' patients, ',
    (SELECT COUNT(DISTINCT site_id) FROM `${PROJECT_ID}.${DATASET_ID}.forecast_site_performance`), ' sites'
